...
{*******************************************************************}
{
 $Id: SkinObjects.pas,v 1.1.1.1 2003/03/21 12:12:44 evgeny Exp $                                                            
}
...
uses
  ...
  Menus, Dialogs, KSDevBmp, SkinConst, SkinTypes, SkinUtils;
...
uses SkinEngine, SkinSource, SkinObjectStore, KSCounter, SkinSound;
...
begin
  ...
  try
    ...
    if Operator = 'play' then
    begin
      Sound := GetSound(TscSkinSource(FSource).Sounds, GetToken(S));
      if Sound <> nil then
        Sound.Play;
    end;
