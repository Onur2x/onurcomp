...
function dyoket(dsy:string):string;  //uzanti ve klasör yok ediliyor
var sonuc:string;
begin
sonuc:='';
...
ShowMessage('Lütfen Resmi Koymak Ýstediðiniz Elamaný Seçin');
exit; end else begin
...
if stili=ortala then Pa:='Kapla' ;
    ...
    Data.WriteString(ObjectName, 'Üst', IntToStr(Top));
    Data.WriteString(ObjectName, 'Geniþlik', IntToStr(Width));
    Data.WriteString(ObjectName, 'Yükseklik', IntToStr(Height));
    Data.WriteString(ObjectName, 'Resim Düzeni',pa);
...
// WriteString(Stream, Suyari);
 { SkinFile := TscStringsIniFile.Create;
  try
    ...
    { Saving general data }
{    SkinFile.WriteString(sGeneralSection, 'Skin Adý', SkinFile.FileName);
...
//       skinfile. WriteString(ObjectName, 'Resim Dosyasý',(FindComponent('o'+inttostr(i+1))as Tpanel).Resim.Bitmap.SaveToStream(stream));
...
ShowMessage('Lütfen Renklendirmek Ýstediðiniz Elamaný Seçin');
exit; end else begin
