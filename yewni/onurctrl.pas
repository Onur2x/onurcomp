...
uses
  ...
  LazUTF8, Zipper, Dialogs,LMessages,{$IFDEF WINDOWS}
  Windows{$ELSE}unix {$ENDIF},types,SysUtils;
...
type
  ...
  private
    ...
    FSkinRead, FFormActive: boolean;
    FRMain, FTempBitmap: TBGRABitmap;
    //clrr: string;
    FFileName: TFileName;
...
begin
  partial := Target.FSkindata.Fimage.GetPart(ARect);
  if partial <> nil then
    Target.resim.StretchPutImage(types.Rect(x, y, w, h), partial,
...
begin
  partial := T.FSkindata.Fimage.GetPart(ARect);
  if partial <> nil then
    Target.StretchPutImage(types.Rect(x, y, w, h), partial,
      dmDrawWithTransparency, Opaque);
  partial.Free;
...
begin
  ...
  begin
    ...
    {  Crp.Croprect.LEFT           := StrToIntDef(myst.Strings[0], 2);
      ...
      Crp.Croprect       := types.Rect(StrToIntDef(myst.Strings[0], 2),StrToIntDef(myst.Strings[1], 4),StrToIntDef(myst.Strings[2], 4),StrToIntDef(myst.Strings[3], 4));

      if Crp.Croprect.IsEmpty then
      Crp.Croprect       := types.Rect(0,0,1,1);
...
//  Fparent.OnDeactivate  := @deactive;
  ...
  FTempBitmap           := TBGRABitmap.Create(self.fparent.clientWidth, Self.fparent.clientHeight);
...
begin
  ...
  begin
    ...
    // Loadskin2;
    Colorbgrabitmap;
//    replacepixel(FTempBitmap, ColorToBGRA(StringToColor(clrr), Fopacity));
  end;
end;
...
begin
  ...
  try
    a.SetSize(FTempBitmap.Width, FTempBitmap.Height);
    replacepixel(FTempBitmap, a, ColorToBGRA(StringToColor(ColorTheme), Opacity));
   // a.InvalidateBitmap;
...
begin
  ...
  TrgtRect := types.Rect(0, 0, FTopleft.Croprect.Width, FTopleft.Croprect.Height);
  DrawPart(FTopleft.Croprect, TrgtRect);


  //TOPRIGHT //SAĞÜST
  TrgtRect := types.Rect(self.fparent.clientWidth - FTopRight.Croprect.Width, 0,
    self.fparent.ClientWidth, FTopRight.Croprect.Height);
  DrawPart(FTopRight.Croprect, TrgtRect);

  //TOP  //ÜST
  TrgtRect := types.Rect(FTopleft.Croprect.Width, 0, self.fparent.ClientWidth -
  ...
  //BOTTOMLEFT // SOLALT
  TrgtRect := types.Rect(0, self.fparent.ClientHeight - FBottomleft.Croprect.Height,
    FBottomleft.Croprect.Width, self.fparent.ClientHeight);
  DrawPart(FBottomleft.Croprect, TrgtRect);

  //BOTTOMRIGHT  //SAĞALT
  TrgtRect := types.Rect(self.fparent.ClientWidth - FBottomRight.Croprect.Width,
    self.fparent.ClientHeight - FBottomRight.Croprect.Height, self.fparent.ClientWidth,
    self.fparent.ClientHeight);
  DrawPart(FBottomRight.Croprect, TrgtRect);

  //BOTTOM  //ALT
  TrgtRect := types.Rect(FBottomleft.Croprect.Width, self.fparent.ClientHeight -
  ...
  //CENTERLEFT // SOLORTA
  TrgtRect := types.Rect(0, FTopleft.Croprect.Height, Fleft.Croprect.Width,
    self.fparent.ClientHeight - FBottomleft.Croprect.Height);
  DrawPart(Fleft.Croprect, TrgtRect);

  //CENTERRIGHT // SAĞORTA
  TrgtRect := types.Rect(self.fparent.ClientWidth - FRight.Croprect.Width, FTopRight.Croprect.Height,
    self.fparent.ClientWidth, self.fparent.ClientHeight - FBottomRight.Croprect.Height);
  DrawPart(FRight.Croprect, TrgtRect);

  //CENTER //ORTA
  TrgtRect := types.Rect(Fleft.Croprect.Width, FTop.Croprect.Height, fparent.ClientWidth -
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FTempBitmap.SetSize(0,0);
          FTempBitmap.putimage(0, 0, Fimage, dmSetExceptTransparent, 255);
...
begin
  ...
  try
    ...
    begin
      ...
      FTempBitmap.SetSize(0,0);
      FTempBitmap.putimage(0, 0, Fimage, dmSetExceptTransparent, 255);  // for color
...
var
  i: Integer;
begin
  if Assigned(Customcroplist) then
  begin
    for i := 0 to Customcroplist.Count - 1 do
      if Assigned(Customcroplist.Items[i]) then
        TONURCUSTOMCROP(Customcroplist.Items[i]).Free;
    FreeAndNil(Customcroplist);
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        on E: Exception do
        begin
          // Error handling - skin okunamazsa devam et
          FSkindata := nil;
        end;
      end;
    end
